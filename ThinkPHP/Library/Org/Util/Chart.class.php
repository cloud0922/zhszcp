...
class Chart {
  ...
  */
  static function create3dpie($title,$data=array(),$size=40,$height=100,$width=80,$legend=array(),$slice=0){
      ...
      // 创建图表
      $imgName=md5($title.'create3dpie').'.png';
	  $graph = new PieGraph($width,$height,$imgName,-1,0);
	  $graph->SetShadow();
	  // 设置标题
  ...
  static function createcolumnar($title,$data=array(),$size=40,$height=100,$width=80,$legend=array()){
		...
		//修改过
		$imgName=md5($title.'createcolumnar').'.png';
		$graph = new Graph($width,$height,$imgName,0,0);
