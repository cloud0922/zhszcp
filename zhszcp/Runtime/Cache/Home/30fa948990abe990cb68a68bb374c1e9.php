...
<div class="easyui-layout" style="width:100%;height:680px;">
		<div data-options="region:'north'" style="height:80px;">
			<center><h4><?php echo ($sname); echo ($pname); ?></h4></center>
		...
		<div data-options="region:'center',split:true,iconCls:'icon-ok'" title="班级：<?php echo ($cname); ?>">
			<table class="table table-hover" style="width:100%;">
				...
				<tbody>
				<form name="form" id="form" action="<?php echo U('Evaluate/QPsubmit');?>" method="post">
		...
		<center>
			...
			<?php else: ?>href="<?php echo U('Evaluate/QP');?>?sl_id=<?php echo ($sl_id); ?>&cid=<?php echo ($cid); ?>&sname=<?php echo ($sname); ?>&cname=<?php echo ($cname); ?>&code=<?php echo ($code); ?>&pname=<?php echo ($pname); ?>&indexp=<?php echo ($lastIndex); ?>"<?php endif; ?>>上一项</a>
			<a class="btn btn-success btn-xs"  <?php if($nextIndex > $indexCount): ?>href="javascript:void(0)" disabled="true" 
			<?php else: ?>href="<?php echo U('Evaluate/QP');?>?sl_id=<?php echo ($sl_id); ?>&cid=<?php echo ($cid); ?>&sname=<?php echo ($sname); ?>&cname=<?php echo ($cname); ?>&code=<?php echo ($code); ?>&pname=<?php echo ($pname); ?>&indexp=<?php echo ($nextIndex); ?>"<?php endif; ?>>下一项</a>
		</center>
...
function fsubmit()
{
	...
	$.ajax({
		type:"post",
		url:"<?php echo U('Evaluate/QPsubmit');?>",
		async:false,
		data: $('#form').serialize(),
		dataType: "json",
		...
		if(msg){
			...
			{
			alert("提交成功,进入下一项！");
			//window.location.href="<?php echo U('Evaluate/QP');?>?sl_id="+sl_id+"&cid=" + cid +"&sname=" +sname +"&cname="+cname+"&code="+code"&pname="+pname +"&indexp="+indexp;
			window.location.href="<?php echo U('Evaluate/QP');?>?sl_id=<?php echo ($sl_id); ?>&cid=<?php echo ($cid); ?>&sname=<?php echo ($sname); ?>&cname=<?php echo ($cname); ?>&code=<?php echo ($code); ?>&pname=<?php echo ($pname); ?>&indexp="+indexp;
			}
			else
			{
